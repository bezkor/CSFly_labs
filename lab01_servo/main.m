...
TF_servo=@(Kpid) feedback(DC_motor*TF_pid(Kpid));
...
% Функція оптимізації
qscore=@(Kpid) norm(TF_servo(Kpid));

% Пошук оптимального рішення
options = optimset('TolX',1e-6,'Display','iter','TolFun',1e-10);
Opt_pid=fminsearch(qscore,[KP,KI,KD])
...
printf("Ручні налаштування :[ ");
printf("%g ",[KP,KI,KD]);
printf("]; ");
printf("Q = %f\n",qscore([KP,KI,KD]));

printf("Оптимізовані налаштування :[ ");
printf("%g ",Opt_pid);
printf("]; ");
printf("Q = %f\n",qscore(Opt_pid));
